...
namespace Tutorial_7_BlockChain.Models
{
    public class BlockChainModel 
    {
        ...
        public static void AddBlock(Block block)
        {
           ...
           // -- This commented out section exists for archival purposes --
           // The invalid hash prefix check returns true for a valid string and I cannot understand why
           /*
            if (!IsLargestID(block.blockID) || block.amount >= GetBalance(block.walletIDFrom)
                || block.amount <= 0 || block.offset % 5 != 0 || block.prevHash != GetLastBlock().currentHash
                || !block.currentHash.StartsWith("12435") || !IsValidHash(block) || !AllFieldsPositive(block))
            {
